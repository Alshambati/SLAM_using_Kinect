...
* (/right_ticks and /left_ticks topics) at regular intervals using the * built-in 
encoder (forward = positive; reverse = negative). 
* The node also subscribes to linear & angular velocity commands published 
on * the /cmd_vel topic to drive the robot accordingly. 
*/ 
...
// Handles startup and shutdown of ROS ros::NodeHandle 
nh; 
...
// True = Forward; False = Reverse boolean 
Direction_left = true; 
boolean Direction_right = true; 
3
 
// Minumum and maximum values for 16-bit integers 
// Range of 65,535 const int 
encoder_minimum = -32768; const int 
encoder_maximum = 32767; 
 
// Keep track of the number of wheel ticks std_msgs::Int16 
...
// Motor A connections 
const int enA = 9; 
const int in1 = 5; const 
int in2 = 6; 
 
// Motor B connections 
const int enB = 10; 
const int in3 = 7; const 
int in4 = 8; 
...
// Distance from center of the left tire to the center of the right tire in m const 
double WHEEL_BASE = 0.17; 
...
const int DRIFT_MULTIPLIER = 00; 
 
// Turning PWM output (0 = min, 255 = max for PWM values) 
const int PWM_TURN = 20; 
 
// Set maximum and minimum limits for the PWM values 
const int PWM_MIN = 20; // about 0.1 m/s const int 
PWM_MAX = 60; // about 0.172 m/s 
 
// Set linear velocity and PWM variable values for each wheel 
double velLeftWheel = 0; double velRightWheel = 0; double 
pwmLeftReq = 0; double pwmRightReq = 0; 
...
static int prevLeftCount = 0; 
 ...
 // Manage rollover and rollunder when we get outside the 16-bit integer range int 
numOfTicks = (65535 + left_wheel_tick_count.data - prevLeftCount) % 65535; 
...
int numOfTicks = (65535 + right_wheel_tick_count.data - prevRightCount) % 
65535; 
...
pwmRightReq = 50; 
 ...
 pwmLeftReq = K_P*abs(cmdVel.angular.z)+b; 
pwmRightReq = -(K_P*abs(cmdVel.angular.z)+b); 
 //pwmLeftReq = PWM_TURN; 
...
// to make sure the robot goes straight static 
double prevDiff = 0; static double 
prevPrevDiff = 0; 
9
...
pwmRightReq += (int)(avgDifference * DRIFT_MULTIPLIER); 
 ...
 // Handle low PWM values if 
(abs(pwmLeftReq) < PWM_MIN) { 
 pwmLeftReq = 0; 
 } 
...
pwmLeftReq = 0; 
 ...
 // Set the direction of the motors if 
(pwmLeftReq > 0) { // Left wheel forward 
...
11
 ...
 // Conditional operator to limit PWM output at the maximum pwmLeftOut = 
(pwmLeftOut > PWM_MAX) ? PWM_MAX : pwmLeftOut; pwmRightOut = 
(pwmRightOut > PWM_MAX) ? PWM_MAX : pwmRightOut; 
 
 // PWM output cannot be less than 0 pwmLeftOut = 
(pwmLeftOut < 0) ? 0 : pwmLeftOut; pwmRightOut = 
(pwmRightOut < 0) ? 0 : pwmRightOut; 
...
pinMode(ENC_IN_LEFT_B , INPUT); pinMode(ENC_IN_RIGHT_A 
, INPUT_PULLUP); pinMode(ENC_IN_RIGHT_B , INPUT); 
 
 // Every time the pin goes high, this is a tick 
12
 attachInterrupt(digitalPinToInterrupt(ENC_IN_LEFT_A), left_wheel_tick, 
RISING); 
 attachInterrupt(digitalPinToInterrupt(ENC_IN_RIGHT_A), right_wheel_tick, 
RISING); 
